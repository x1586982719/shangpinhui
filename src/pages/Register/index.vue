  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button @click="getCode" style="width:80px;height:38px">获取验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
      ...
      <div class="controls">
        ...
        <span class="error-msg">请先同意协议再提交注册</span>
      </div>
      <div class="btn">
...
export default {
  ...
  methods: {
    async getCode() {
      if (this.phone) {
        await this.$store.dispatch('verCode', this.phone) //这需要做异步处理，等待服务器返回验证码后再赋值data
        this.code = this.$store.state.User.verCode
      }
    },
    async submit() {
      ...
      if (phone && password == password1 && code) {
        try {
          await this.$store.dispatch('submit', { phone, password, code })
