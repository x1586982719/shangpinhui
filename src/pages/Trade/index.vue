<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="ad in address" :key="ad.id">
        <span class="username" :class="{ selected: ad.isDefault == '1' }" @click="selected(ad)">{{ ad.consignee
        }}</span>
        <p>
    ...
    <div class="trade">
      <div class="price">应付金额:　<span>¥{{ amount }}</span></div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      let { consignee, phoneNum, fullAddress } = this.currentAddress
      let { orderComment, detailArrayList } = this
  ...
  computed: {
    ...
    amount() {
      ...
      this.detailArrayList.forEach(item => {
        sum = sum + item.orderPrice * item.skuNum
      });
      return sum
    },
  },
